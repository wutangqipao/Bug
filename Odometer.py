...
class LS7366R():
    ...
    def __init__(self, CSX, CLK, BTMD):
        ...
        self.spi.open(1, 0)  # Which CS line will be used
        self.spi.max_speed_hz = self.CLK  # Speed of clk (modifies speed transaction)
        # Init the Encoder
        ...
        self.spi.close()
        self.spi.open(1, 1)  # Which CS line will be used
        self.spi.max_speed_hz = self.CLK  # Speed of clk (modifies speed transaction)
        # Init the Encoder
        print('Clearing Encoder CS%s\'s Count...\t' % (str(CSX)), self.clearCounter())
        print('Clearing Encoder CS%s\'s Status..\t' % (str(CSX)), self.clearStatus())
        ...
        self.spi.close()
        self.spi.open(1, 2)  # Which CS line will be used
        self.spi.max_speed_hz = self.CLK  # Speed of clk (modifies speed transaction)
        # Init the Encoder
        print('Clearing Encoder CS%s\'s Count...\t' % (str(CSX)), self.clearCounter())
        print('Clearing Encoder CS%s\'s Status..\t' % (str(CSX)), self.clearStatus())
    ...
    def Switch_device(self, CSX):
        # print(0)
        self.spi.close()
        self.spi.open(1, CSX)
        self.spi.max_speed_hz = self.CLK
